...
public class dbGEOUtils {
    ...
    public dbGEOUtils(Context ctx) {
        context = ctx;
        conn = new DBHelper(ctx);
    }

    public List<GEO> get_GEO_name1() {
        List<GEO> data = new ArrayList<GEO>();
        SQLiteDatabase db = conn.getWritableDatabase();
        Cursor c = db.rawQuery("SELECT g1, g2, g3, name1, name2, name3 FROM set_geo_codes ",null);
        if (c != null) {
