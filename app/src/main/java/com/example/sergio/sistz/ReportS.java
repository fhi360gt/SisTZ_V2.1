...
// ***************** Load Teacher/Staff LIST *************************
    public void loadListTeacher(String sql) {
        ...
        SQLiteDatabase dbSET = cnSET.getReadableDatabase();
        Cursor cur_data = dbSET.rawQuery("SELECT DISTINCT(s._id), s.surname, s.givenname, s.family FROM student  AS s \n" +
                                        "INNER JOIN _sa  AS sa ON sa.sc=s._id " + sql + " AND year_ta=" + school_year + " \n" +
                                        "GROUP BY s._id, s.surname, s.givenname, s.family ORDER BY s.family, s.surname", null);
